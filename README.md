...
- **`WebAppApi`** (HTTP API) + 8 Lambdas:
  ...
  | `GET /api/documents/{documentId}/view` | `view_document` | Returns `{"viewUrl": ...}` — a short-lived (5 min) presigned S3 GET URL for the original uploaded PDF, with response headers forcing inline rendering (`ResponseContentType: application/pdf`, `ResponseContentDisposition: inline`). Every route requires a JWT, so the frontend calls this with `fetch` (a bare `<a>` navigation can't carry an `Authorization` header) — but the URL comes back as JSON rather than a 302 redirect, since a `fetch` that *follows* a cross-origin redirect needs the final (S3) response to carry CORS headers too, which `RawDocumentsBucket`'s CORS policy doesn't grant for GET. The frontend instead navigates a new tab directly to `viewUrl`, a plain top-level navigation that isn't CORS-checked at all — PDF bytes still never get proxied through API Gateway/Lambda. |
